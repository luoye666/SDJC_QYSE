			...
			switch (s.eventType) {
				case "hover": {
					$(this).hover(function() {		
						...
						var numShowDelay = parseInt(s.showDelay, 100);
						init(s, $(this));
						//鼠标hover延时
						if (numShowDelay) {
							if (hoverTimer) {
	...
	var o = {
		targetGet: function() {
			...
			switch (this.s.targetMode) {
				...
				case "list": {
					//下拉列表
					var targetHtml = '<table id="grid" class="table"  border="0" cellspacing="0" cellpadding="0">',  arrLength;
					targetHtml += ' <tr> <td>主键</td> <td>使用单位</td> <td>注册代码</td> </tr>';
					if ($.isArray(target) && (arrLength = target.length)) {
						$.each(target, function(i, obj) {
							var list = "", strClass = "", text, href ,id ,sydw;
							if (i === 0) {
							...
							if (typeof(obj) === "object" && (text = obj.text.toString()) && (id = obj.id.toString()) && (sydw = obj.sydw.toString())) {
							
									list = '<td>'+ id +'</td> <td>'+text+'</td> <td>'+sydw+'</td>';
								
							} else if (typeof(obj) === "string" && obj) {
								list = obj;
							}
							if (list) {
								targetHtml += '<tr >' + list + '</tr>';	
							}
						});
					} else {
