...
var THISPAGE = {
	...
	loadGrid: function(){
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'jgdm', label:'联系电话', index:'jgdm', width:100,align:"center"},
						{name:'frtel', label:'联系人', index:'frtel', width:100,align:"center"},
						{name:'fid', label:'资质证书', index:'fid', width:100,align:"center",
			        		formatter:function(value,opt, row){
			        			value = '<a href="javascript:void(0)" onclick="editZzzs('+row.id+',\''+row.dwmc+'\')">资质证书</a>';
								return value;
			        	}},
		...
		_self.markRow = [];
		$("#grid").jqGrid({
			url:path+'/scUtilsList!getList.do',
