...
var THISPAGE = {
	...
	loadGrid: function(){
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'name', label:'企业名称', index:'name', width:120, align:"center"},
						{name:'filePath' , label:'资料', index:'filePath', width:70, align:"center",
							formatter:function(value,opt,row){
								// <a href="javascript:void(0)" onclick="editQyZyry('+row.id+',\''+row.dwmc+'\')">详细人员</a>
								if(row.type == '4'){
									return '<a href="javascript:void(0)" onclick="openWin(\''+row.filePath+'\','+row.type+')">学习网站</a>';
								}else if(row.type == '3'){
									return '<a href="javascript:void(0)" onclick="openVed(\''+row.filePath+'\','+row.type+')">播放</a>';
								}else{
									return '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-circle-arrow-s" onclick="openFile(\''+row.filePath.replace(/\\/g,"/")+'\','+row.type+')" title="下载资料"></span></div>';
								}
		...
		_self.markRow = [];
		$("#grid").jqGrid({
			url:path+'/syUtilsList!findUseData.do?type='+type,
	...
	//		scroll: 1, //创建一个动态滚动的表格，当为true时，翻页栏被禁用，使用垂直滚动条加载数据，且在首次访问服务器端时将加载所有数据到客户端。当此参数为数字时，表格只控制可见的几行，所有数据都在这几行中加载
		...
		function operFmatter (val, opt, row) {//val：要被格式化的值  options：对数据进行格式化时的参数设置，格式为： { rowId: rid, colModel: cm} rowObject：行数据
			var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-pencil" onclick="editInspect('+row.id+')" title="修改"></span></div>';
			return html_con;
		};
