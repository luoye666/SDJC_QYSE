...
var THISPAGE = {
	...
	loadGrid: function(){
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'zsbh', label:'许可证编号', index:'zsbh', width:120, align:"center"},
						{name:'xkzfwh', label:'许可证发文号', index:'xkzfwh', width:100,align:"center"},
						{name:'xkzlb', label:'许可类别', index:'xkzlb', width:100, align:"center"},
						{name:'pzjb', label:'批准级别', index:'pzjb', width:100,align:"center"},
						{name:'zsyxq_js', label:'有效期', index:'zsyxq_ks', hidden:true},
						{name:'zsyxq_ks', label:'证书有效期', index:'zsyxq_js', width:100,align:"center",
							formatter:function(value,opt,row){
								return SYSTEM.date.format(value,'yyyy-MM-dd')+"-"+SYSTEM.date.format(row.zsyxq_js,'yyyy-MM-dd');
						}},
						{name:'operating', label:'查看',width:80, fixed:true, formatter:operFmatter, align:"center"}
						]; 
		_self.markRow = [];
		$("#grid").jqGrid({
			url:path+'/scUtilsListLi!getList.do?fid='+fid,
	...
	//		scroll: 1, //创建一个动态滚动的表格，当为true时，翻页栏被禁用，使用垂直滚动条加载数据，且在首次访问服务器端时将加载所有数据到客户端。当此参数为数字时，表格只控制可见的几行，所有数据都在这几行中加载
		...
		function operFmatter (val, opt, row) {//val：要被格式化的值  options：对数据进行格式化时的参数设置，格式为： { rowId: rid, colModel: cm} rowObject：行数据
			var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-pencil" onclick="editScLi('+row.id+')" title="修改"></span></div>';
			return html_con;
		};
