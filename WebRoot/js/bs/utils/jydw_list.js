...
var THISPAGE = {
	...
	loadGrid: function(){
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'shi', label:'所在市', index:'shi', width:120, align:"center"},
						{name:'quxian', label:'所在区县', index:'quxian', width:120, align:"center"},
						{name:'frdb', label:'法人代表', index:'frdb', width:100,align:"center"},
						{name:'jynr', label:'检验内容', index:'jynr', width:100, align:"center"},
						{name:'zsyxq', label:'证书有效期', index:'zsyxq', width:100,align:"center",
		...
		_self.markRow = [];
		$("#grid").jqGrid({
			url:path+'/jyUtilsList!getList.do',
	...
	//		scroll: 1, //创建一个动态滚动的表格，当为true时，翻页栏被禁用，使用垂直滚动条加载数据，且在首次访问服务器端时将加载所有数据到客户端。当此参数为数字时，表格只控制可见的几行，所有数据都在这几行中加载
		...
		function operFmatter (val, opt, row) {//val：要被格式化的值  options：对数据进行格式化时的参数设置，格式为： { rowId: rid, colModel: cm} rowObject：行数据
			var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-pencil" onclick="editJy('+row.id+')" title="修改"></span></div>';
			return html_con;
		};
