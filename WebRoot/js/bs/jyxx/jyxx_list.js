...
var THISPAGE = {
	...
	loadGrid: function(){
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'shibaiyuanyin', label:'更新失败原因', index:'shibaiyuanyin', width:100,align:"center"},
						{name:'operating', label:'查看',width:50, fixed:true, formatter:operFmatter, align:"center"}
						]; 
		_self.markRow = [];
		$("#grid").jqGrid({
			url:path+'/jyxxList!getList.do?gxstate='+gxstate+'&info='+info,
	...
	//		scroll: 1, //创建一个动态滚动的表格，当为true时，翻页栏被禁用，使用垂直滚动条加载数据，且在首次访问服务器端时将加载所有数据到客户端。当此参数为数字时，表格只控制可见的几行，所有数据都在这几行中加载
			...
			gridComplete:function(){
				if("wgx" == info){
					$("#grid").jqGrid('setGridParam',{}).jqGrid('hideCol',["gengxinshijian","shibaiyuanyin"]);
					$("#grid").setGridWidth($(window).width()-30);
				}else if("ygx" == info){
					$("#grid").jqGrid('setGridParam',{}).jqGrid('hideCol',["shibaiyuanyin"]);
					$("#grid").setGridWidth($(window).width()-30);
				}
			}
