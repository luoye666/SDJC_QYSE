...
var THISPAGE = {
	...
	loadGrid: function(){
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'shinew', label:'所在市', index:'shinew', width:120, align:"center"},
						{name:'ouidnamenew', label:'所在区县', index:'ouidnamenew', width:120, align:"center"},
						{name:'confirmflag', label:'操作', index:'confirmflag', width:80, align:"center",
	...
	//		scroll: 1, //创建一个动态滚动的表格，当为true时，翻页栏被禁用，使用垂直滚动条加载数据，且在首次访问服务器端时将加载所有数据到客户端。当此参数为数字时，表格只控制可见的几行，所有数据都在这几行中加载
		...
		function operFmatter (val, opt, row) {//val：要被格式化的值  options：对数据进行格式化时的参数设置，格式为： { rowId: rid, colModel: cm} rowObject：行数据
			var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-pencil" onclick="editMsg(\''+row.id+'\',\''+row.idold+'\','+row.confirmflag+')" title="编辑"></span></div>';
			return html_con;
		};
