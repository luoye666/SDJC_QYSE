...
var THISPAGE = {
	...
	loadGrid: function(){
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'organid', label:'organid', index:'organid', width:120, align:"center",hidden:true},
						{name:'orgname', label:'申请人名称', index:'orgname', width:140, align:"center"},
						{name:'organno', label:'组织机构代码', index:'organno', width:80, align:"center"},
						{name:'abode', label:'住所', index:'abode', width:160, align:"center"},
						{name:'region', label:'行政区划', index:'region', width:100, align:"center"},
						{name:'itemname', label:'申请类别', index:'itemname', width:120, align:"center"},
						{name:'certno', label:'证书编号', index:'certno', width:100, align:"center"},
						{name:'sdate', label:'发证日期', index:'sdate', width:80, align:"center",formatter:function(value){
						...
						{name:'edate', label:'有效期', index:'edate', width:80, align:"center",formatter:function(value){
							if("" == value || undefined == value){
								return "";
							}else{
								return SYSTEM.date.format(value,'yyyy-MM-dd');
	...
	//		scroll: 1, //创建一个动态滚动的表格，当为true时，翻页栏被禁用，使用垂直滚动条加载数据，且在首次访问服务器端时将加载所有数据到客户端。当此参数为数字时，表格只控制可见的几行，所有数据都在这几行中加载
		...
		function operFmatter (val, opt, row) {//val：要被格式化的值  options：对数据进行格式化时的参数设置，格式为： { rowId: rid, colModel: cm} rowObject：行数据
			var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-pencil" onclick="editSc('+row.id+','+row.organid+',\''+row.itemname+'\')" title="修改"></span></div>';
			return html_con;
		};
