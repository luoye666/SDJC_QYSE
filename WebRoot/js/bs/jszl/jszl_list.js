...
var THISPAGE = {
	...
	loadGrid: function(){
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'regulationstype', label:'文件类型', index:'regulationstype', width:120, align:"center"},
						{name:'setypename', label:'设备类型', index:'setypename', width:80, align:"center"},
						{name:'filename', label:'文件名称', index:'filename', width:100,align:"center"},
						{name:'state', label:'文件状态', index:'state', width:100, align:"center",
						...
						{name:'fbrq' , label:'发布日期', index:'fbrq', width:100, align:"center",
							formatter:function(value){
								if(undefined == value){
									return "";
								}else{
						...
						{name:'ssrq' , label:'实施日期', index:'ssrq', width:100, align:"center",
							formatter:function(value){
								if(undefined == value){
									return "";
								}else{
		...
		_self.markRow = [];
		$("#grid").jqGrid({
			url:path+"/jszlList!getList.do?seCode="+seCode,
