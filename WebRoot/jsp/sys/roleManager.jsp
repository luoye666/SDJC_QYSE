...
<div class="wrapper">
	  <div class="mod-search cf">
	    <a class="ui-btn ui-btn-sp" id="mnubtn_addRole">添加</a><a class="ui-btn ui-btn-sp" id="mnubtn_delRole">删除</a>
...
var path = "<%=path %>";
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'name', label:'角色名(中)', index:'name',align:"center"},
						{name:'nameEN' , label:'角色名(英)', index:'nameEN', width:100, align:"center"},	
						...
						{name:'operating', label:'操作',width:80, fixed:true, align:"center",
			        		formatter: function(val, opt, row){//val：要被格式化的值  options：对数据进行格式化时的参数设置，格式为： { rowId: rid, colModel: cm} rowObject：行数据
								...
								value += '<span class="ui-icon ui-icon-pencil" onclick="editRole('+row.id+')" title="修改"></span>';
		...
		function editUsersRole(id){
				$.dialog({
					    ...
					    height: '450px',
					    lock: true,
					    content: 'url:<%=path%>/jsp/sys/roleManager_editUsersRole.jsp?id='+id
				});
		}
		...
		$().ready(function(){
			...
			$("#mnubtn_addRole").click(function(){
				$.dialog({
					    ...
					    height: '350px',
					    lock: true,
					    content: 'url:<%=path%>/jsp/sys/roleManager_addRole.jsp'
				});
			});
