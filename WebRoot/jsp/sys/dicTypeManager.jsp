...
<div class="wrapper">
  <div class="mod-search cf">
    <a class="ui-btn ui-btn-sp" id="mnubtn_addDicType">添加</a><a class="ui-btn ui-btn-sp" id="mnubtn_delDicType">删除</a>
...
var path = "<%=path %>";
		...
		var colModels = [//fixed列宽度是否要固定不可变
						...
						{name:'name', label:'类型名称', index:'name',align:"center"},
						{name:'code' , label:'类型编号', index:'code', width:100, align:"center"},	
						{name:'notes', label:'类型描述', index:'notes', width:100, align:"center"},
						{name:'operating', label:'操作',width:80, fixed:true, align:"center",
			        		formatter: function(val, opt, row){//val：要被格式化的值  options：对数据进行格式化时的参数设置，格式为： { rowId: rid, colModel: cm} rowObject：行数据
								var value = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-pencil" onclick="editDicType('+row.id+')" title="修改"></span></div>';
								return value;
						}}
						]; 

		function editDicType(id){
				$.dialog({
						title:'详细信息',
					    width: '450px',
					    height: '250px',
					    lock: true,
					    content: 'url:<%=path%>/dic!openZdTypeInfo.do?id='+id
				});
		}
		...
		$().ready(function(){
			...
			$("#grid").jqGrid({
				url:"<%=path%>/dicList!getTypeList.do",
				datatype: "json",
				mtype:"post",
				autowidth: true,
				height: height,
		...
		$().ready(function(){
			/**功能按钮事件***********************************************************************************************************************/
			//添加字典按钮点击
			$("#mnubtn_addDicType").click(function(){
				$.dialog({
						title:'添加字典类型',
					    width: '450px',
					    height: '250px',
					    lock: true,
					    content: 'url:<%=path%>/jsp/sys/dicTypeManager_addDicType.jsp'
				});
			});
			
			//删除字典按钮点击
			$("#mnubtn_delDicType").click(function(){
				...
				$.dialog.confirm("确定要删除选择的字典类型吗?", function(){  
	                $.post("<%=path%>/dic!deleteTypeByIds.do?ids="+ids,{},
